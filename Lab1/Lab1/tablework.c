...
int checkRawCond(struct FileMapping* Maping, struct queryCondition* condition, struct raw_sequence* ret) {
	...
	if (condition->type == INT_DATA) {
		...
		case GREATER: return ret->raw_data[condition->stolbec_num].int_data > condition->int_data;
		case LESS: return ret->raw_data[condition->stolbec_num].int_data < condition->int_data;
		case NOT_EQUAL: return ret->raw_data[condition->stolbec_num].int_data == condition->int_data;
		case EQUAL: return ret->raw_data[condition->stolbec_num].int_data == condition->int_data;
		default: return 0;
		}
	}
	...
	if (condition->type == DOUBLE_DATA) {
		switch (condition->conds) {
		case GREATER: return ret->raw_data[condition->stolbec_num].double_data > condition->double_data;
		case LESS: return ret->raw_data[condition->stolbec_num].double_data < condition->double_data;
		case NOT_EQUAL: return ret->raw_data[condition->stolbec_num].double_data == condition->double_data;
		case EQUAL: return ret->raw_data[condition->stolbec_num].double_data == condition->double_data;
		default: return 0;
		}
	}
	...
	if (condition->type == STRING_DATA) {
		...
		int ret_data = 1;
		switch (condition->conds) {
		case EQUAL: ret_data = strcmp(string_from_file, condition->string_data);
		case NOT_EQUAL: ret_data = !(strcmp(string_from_file, condition->string_data));
		default: ret_data = 0;
		}
			...
			if(checkJoinEqual(&ret1_cop->raw_data[join_cond.first_column], &ret2_cop->raw_data[join_cond.second_column])){
				...
				for (int i = 0; i < ret1->len; i++) {
					...
					if(temp->raw_data[i].flag == STRING_DATA){
						temp->raw_data[i].string_link.string_data = malloc(sizeof(char) * ret1_cop->raw_data[i].string_link.str_len);
						for (int j = 0; j < ret1_cop->raw_data[i].string_link.str_len; j++)
							temp->raw_data[i].string_link.string_data[j] = ret1_cop->raw_data[i].string_link.string_data[j];
					}
				}
				for (int i = 0; i < ret2->len; i++) {
					...
					if (temp->raw_data[ret1->len + i].flag == STRING_DATA) {
						temp->raw_data[ret1->len + i].string_link.string_data = malloc(sizeof(char) * ret2_cop->raw_data[i].string_link.str_len);
						for (int j = 0; j < ret2_cop->raw_data[i].string_link.str_len; j++)
							temp->raw_data[i].string_link.string_data[j] = ret2_cop->raw_data[i].string_link.string_data[j];
					}
				}
				ans = temp;
			}
